...
ajax('/order/admin/stats', {}, function(res) {
    if (res.code == 0) {
...
ajax('/order/admin/payment-types', {}, function(res) {
    if (res.code == 0) {
...
ajax('/order/admin/detail', {id: row.id}, function(res) {
    if (res.code == 0) { 
...
ajax('/order/admin/payment-info', {order_id: row.id}, function(res) {
    if (res.code == 0) {
...
ajax('/order/admin/update-status', app.form, function(res) {
    " . vue_message() . "
    if (res.code == 0) {
...
ajax('/order/admin/add-payment', app.paymentForm, function(res) {
    " . vue_message() . "
    if (res.code == 0) {
...
app.logisticForm.order_num = row.order_num;
ajax('/order/admin/logistic-info', {order_num: row.order_num}, function(res) {
    if (res.code == 0) {
        app.logisticForm.no = res.no || '';
        app.logisticForm.type = res.type || '';  
    }else{
        app.logisticForm.no = '';
        app.logisticForm.type = '';
    }
}); 
app.logisticDialogVisible = true;
");

// 提交物流信息
$vue->method("submitLogistic()", "
let url = app.logisticForm.id ? '/order/admin/edit-logistic' : '/order/admin/add-logistic';
ajax(url, app.logisticForm, function(res) {
    " . vue_message() . "
    if (res.code == 0) {
        app.logisticDialogVisible = false;
        app.load_list();
    }
});
");
...
// 查看物流信息
$vue->method("viewLogistic(row)", "
ajax('/order/admin/logistic-info', {order_num: row.order_num}, function(res) {
    if (res.code == 0) {
        app.logisticInfo = res;
        app.logisticData = res.data;  
        app.logisticInfoDialogVisible = true;
    } else {
        app.\$message.warning(res.msg || '该订单暂无物流信息');
    }
});
");
...
// 更新物流状态
$vue->method("updateLogisticStatus(id, status)", "
ajax('/order/admin/update-logistic', {id: id, status: status}, function(res) {
    " . vue_message() . "
    if (res.code == 0) {
        app.load_list();
        app.logisticInfoDialogVisible = false;
    }
});
");

// 权限控制
$vue->data("can_view", has_access('order/admin/detail'));
$vue->data("can_edit", has_access('order/admin/updateStatus'));
$vue->data("can_payment", has_access('order/admin/addPayment'));
                ...
                [
                    ...
                    'tpl' => [
                        ['type' => 'html', 'html' => ' 
                            ...
                            <el-button 
                                v-if="can_edit && (scope.row.status == \'paid\' || scope.row.status == \'shipped\')" 
                                type="text" 
                                size="small" 
                            ...
                            <el-button 
                                v-if="can_view && scope.row.status == \'shipped\'" 
                                type="text" 
                                size="small" 
                                @click="viewLogistic(scope.row)"
                            >
                                <i class="el-icon-location"></i>' . lang("物流") . '</el-button>
...
<el-dialog title="<?php echo lang('物流信息'); ?>" top="20px" :visible.sync="logisticInfoDialogVisible" width="800px">
    <div v-if="logisticInfo.no">
            ...
            <div class="col-md-6">
                ...
                <table class="table table-bordered">
                    ...
                    <tr>
                        ...
                        <td v-if="logisticInfo.data">{{ logisticInfo.data.status || '' }}</td>
                    </tr>
                    <tr>
                        <td><?php echo lang('创建时间'); ?></td>
        ...
        <div v-if="logisticData && logisticData.list.length > 0">
