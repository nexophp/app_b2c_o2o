...
class AlipayQuery
{
    public static function run($outTradeNo)
    {
        /*** 请填写以下配置信息 ***/
        $appid = get_config("alipay_appid");  //https://open.alipay.com 账户中心->密钥管理->开放平台密钥，填写对应应用的APPID 
        ...
        add_log('支付宝查询支付状态', $res, 'info');
        /**
         * https://opendocs.alipay.com/apis/api_1/alipay.trade.query
         */
        if ($res['code'] != '10000') {
            $err = $res['msg'] . '：' . $res['sub_code'] . ' ' . $res['sub_msg'];
            json_error(['msg' => $err]);
        } else {
            switch ($res['trade_status']) {
                case 'WAIT_BUYER_PAY':
                    json_error(['msg' => '交易创建，等待买家付款']);
                    break;
                case 'TRADE_CLOSED':
                    json_error(['msg' => '未付款交易超时关闭，或支付完成后全额退款']);
                    break;
                case 'TRADE_SUCCESS':
                    //交易支付成功
                    $data = [
                        ...
                        'openid'   => $res['buyer_user_id'],
                        'paid_at'  => $res['gmt_payment'],
                        'app_id'   => '',
                        'type'     => 'alipay',
                    ];
                    add_log('query支付宝回调', $data, 'debug');
                    do_action('payment_success', $data);
                    return ['is_paid' => 1];
                    break;
                case 'TRADE_FINISHED':
                    json_error(['msg' => '交易结束，不可退款']);
                    break;
                default:
                    json_error(['msg' => '未知状态']);
                    break;
