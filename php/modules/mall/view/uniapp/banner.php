    ...
    <div id="banner-images-container" v-if="pageElements[selectedElement].config && pageElements[selectedElement].config.length > 0">
        ...
        <draggable v-model="pageElements[selectedElement].config">
            ...
            <div v-for="(v, imgIndex) in pageElements[selectedElement].config" class="ele-box banner-image-item mb-3 p-3 border rounded" >
                <div class="mb-2">
                    <img @click="selectBannerImage(imgIndex)" v-if="v.image" :src="v.image" style="width: 100px; height: 100px; object-fit: cover;" class="border rounded">
                    <div @click="selectBannerImage(imgIndex)" v-else class="bg-light border rounded d-flex align-items-center justify-content-center" style="width: 100px; height: 60px;">
                ...
                <button class="btn btn-sm btn-outline-danger ele-del" @click="removeBannerImage(imgIndex)">删除</button>
            </div>
        </draggable>
    </div>
    <button class="btn btn-sm btn-primary" @click="addBannerImage()">添加图片</button>
</div>

<?php
$vue->method("addBannerImage()", " 
...
$vue->method("selectBannerImage(index)", "
...
$vue->method("removeBannerImage(index)","
