...
ajax('/webpos/admin/stats', {}, function(res) {
    if (res.code == 0) {
...
ajax('/webpos/admin/payment-types', {}, function(res) {
    if (res.code == 0) {
...
ajax('/webpos/admin/detail', {id: row.id}, function(res) {
    if (res.code == 0) { 
...
ajax('/webpos/admin/payment-info', {order_id: row.id}, function(res) {
    if (res.code == 0) {
...
ajax('/webpos/admin/update-status', app.form, function(res) {
    " . vue_message() . "
    if (res.code == 0) {
...
ajax('/webpos/admin/add-payment', app.paymentForm, function(res) {
    " . vue_message() . "
    if (res.code == 0) {
...
layer.confirm('确定发货吗？', {
    ...
    ajax('/webpos/admin/add-logistic', {id: row.id}, function(res) {
        " . vue_message() . "
        app.load_list();
        layer.closeAll();
    });  
...
layer.confirm('确定订单完成吗？', {
    ...
    ajax('/webpos/admin/complete', {id: row.id}, function(res) {
        " . vue_message() . "
        app.load_list();
        layer.closeAll();
    });  
...
// 权限控制
$vue->data("can_view", has_access('webpos/admin/detail'));
$vue->data("can_edit", has_access('webpos/admin/updateStatus'));
$vue->data("can_payment", has_access('webpos/admin/addPayment'));
