			...
			<view class="order-container">
				<view v-for="order in orderList" :key="order.id" class="order-item" @click="goToOrderDetail(order.id)">
					...
					<view class="product-list">
						<view v-for="product in order.products"   class="product-item"> 
							<view class="product-info">
								<text class="product-name">{{ product.title }}</text>
								<text class="product-spec">{{ product.str_1 || '' }} {{ product.str_2 || '' }}</text>
					...
					<view class="order-footer">
						<text class="total-amount">共{{ order.num }}件商品 合计：¥{{ order.real_amount }}</text>
...
export default {
	...
	methods: {
		...
		load() {
			let w = this.where
			_this.ajax(_this.config.service.order.index, w).then(res => {
		...
		goToOrderDetail(orderId) {
			uni.navigateTo({
				url: `/pages/service/order_view?id=${orderId}`
			})
		},
