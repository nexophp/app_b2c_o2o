<template>
	<view class="confirm-container">
		...
		<!-- 底部提交按钮 -->
		<view v-if="store.can_buy">
			<view class="bottom-submit bottom-safe-area" v-if="confirmData.real_amount>0">
		...
		<view v-else class="bottom-submit bottom-safe-area" style="text-align: center;">
			<view class="total-text" style="width: 100%;" v-if="store.status != 1">暂未营业</view>
			<view class="total-text" style="width: 100%;" v-else>不在营业时间段</view>
...
<script>
	export default {
		...
		methods: {
			...
			delete_selected(){
				this.ajax(this.config.cart.delete_selected,{})
			},
			// 提交订单
			submitOrder() {
