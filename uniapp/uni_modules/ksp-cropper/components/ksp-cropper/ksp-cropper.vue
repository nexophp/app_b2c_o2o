<template>
<view  :mode="modeValue" :change:mode="mwx.changeMode" :rotate="rotate" :change:rotate="mwx.changeRotate">
	<!-- #ifdef MP-WEIXIN -->
	<canvas v-if="is_load" type="2d" class="canvas" :style="{width: target.width + 'px', height: target.height + 'px'}"></canvas>
	<!-- #endif -->
	<!-- #ifdef APP-PLUS || H5 -->
	<canvas v-if="is_load" :canvas-id="canvasId" :style="{width: target.width + 'px', height: target.height + 'px'}"></canvas>
	<!-- #endif -->
	<view class="panel">
		<view class="body" @touchstart="mwx.touchstart" @touchmove="mwx.touchmove" @touchend="mwx.touchend" @touchcancel="mwx.touchcancel">
			<view class="image-wrap"   :class="{transit: transit}" :change:rect="mwx.changeImage" :rect="image">
				<image class="image" :class="{transit: transit,is_hide:is_hide}" :src="url" @load="imageLoad"/>
			</view>
			<view class="mask"></view>
			<view v-if="is_load" class="frame" :class="{transit: transit}" :change:rect="mwx.changeFrame" :rect="frame">
				<view class="rect">
